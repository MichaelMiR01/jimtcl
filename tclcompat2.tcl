...
# overwrite load, since it only accepts one param hardcoded
catch {
rename load __load
proc load {libp args} {
	if {[catch {__load $libp} e]} {error $e}
}
}
...
proc loadLocalRC {} {
    # 
    set files {jimrc.tcl .jimrc}
    set paths [list ::jim::argv0 ::jim::exe]
    foreach path $paths {
        if {[exists $path]} {
            set path [file dir [set $path]]
            foreach fname $files {
                #puts "lookup $path $fname"
                if {[file exists [file join $path $fname]]} {
                    source [file join $path $fname]
                    return
                }
