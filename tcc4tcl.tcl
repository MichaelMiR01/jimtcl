...
# ported to jim 0.83 by MiR 2025
...
namespace eval ::tcc4tcl {
	...
	proc _proc {handle tclname adefs rtype body args} {
        ...
        # moreover, under windows this works, und linux jim complains about not finding the proc
	...
	proc _go {handle {outputOnly 0}} {
		...
		switch -- $state(type) {
			...
			"package" {
				...
				append code "  if (Jim_CheckAbiVersion(interp, JIM_ABI_VERSION) == JIM_ERR ||\\\n"
				append code "    Jim_PackageProvide(interp,  \"$packageName\", \"$packageVersion\", JIM_ERRMSG)) \{\\\n"
				append code "       printf(\"Error loading Package %s, ABI version mismatch, got %d \\n\",\"$packageName\",JIM_ABI_VERSION);\n"
				append code "       printf(\"Update yout includes.zip with the right jim.h\\n\");\n"
    				append code "       return JIM_ERR; \n"
                 append code "\}\n\n"
